...
public class ClientInvoices extends Activity
{
    ...
    {
        Cursor cursor = myDb.getCustomerInvoice(customer_ID); // Create the list
                                                              // of items
        ...
        list.setOnItemClickListener(new AdapterView.OnItemClickListener()
        {
                    ...
                    int position, long idInDB)
            {
                ...
                if (cursor.moveToFirst())
                {
                    ...
                    String issuedate = cursor
                            .getString(InvoiceAdapter.COL_ISSUEDATE);
                    String dateserviceperformed = cursor
                            .getString(InvoiceAdapter.COL_DATESERVICEPERFORMED);
                    String servicedesc = cursor
                            .getString(InvoiceAdapter.COL_SERVICEDESC);
                    String status = cursor.getString(InvoiceAdapter.COL_STATUS);
                    String priceservice = cursor
                            .getString(InvoiceAdapter.COL_PRICESERVICE);

                    String message = "Rowid: " + idDB + "\n" + customer + "\n"
                            + issuedate + "\n" + dateserviceperformed + "\n"
                            + servicedesc + "\n" + status + "\n" + priceservice
                            + "Contact Info: " + customer_fname + " "
                            + customer_lname + "\n" + customer_address + "\n"
                            + customer_phone + "\n" + customer_email;
                    Toast.makeText(ClientInvoices.this, message,
                            Toast.LENGTH_LONG).show();
                } else
                    Toast.makeText(ClientInvoices.this,
                            "failed to load " + idInDB, Toast.LENGTH_SHORT)
                            .show();
                cursor.close();
    ...
    public void onClick_AddInvoice(View v)
    {
        ...
        myDb.insertRow(issuedate, customer, dateserviceperformed, priceservice,
                servicedesc, amountdue, status);
